...
public class PickupListener implements Listener {
    ...
    public void onItemDrop(PlayerDropItemEvent event) {
        ...
        if (isTerraWand(dropped) && config.deleteWandOnDrop) {
    ...
    public void onInventoryDrag(InventoryDragEvent event) {
        ...
        if (!isTerraWand(draggedItem) && !config.deleteWandOnStore) return;
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (cursor != null && isTerraWand(cursor) && isContainer && config.deleteWandOnStore) {
            runLater(player, () -> {
                removeWandFromInventory(topInventory);
                handleWandLoss(player);
            });
        ...
        if (event.getClick().isShiftClick() && currentItem != null && isTerraWand(currentItem) && config.deleteWandOnStore) {
            runLater(player, () -> {
                removeWandFromInventory(topInventory);
                handleWandLoss(player);
            });
    ...
    public void onInteract(PlayerInteractEntityEvent event) {
        ...
        if (!isTerraWand(hand) || !config.deleteWandOnStore) return;
    ...
    public void onItemPickup(EntityPickupItemEvent event) {
        ...
        if (isTerraWand(item) && config.deleteWandOnPickup) {
            event.setCancelled(true);
    ...
    public void onBowShoot(EntityShootBowEvent event) {
        ...
        if (isTerraWand(arrow) && config.deleteWandOnShot) {
            event.setCancelled(true);
