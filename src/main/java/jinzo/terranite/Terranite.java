...
public final class Terranite extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if ((command = getCommand("s")) != null) {
            command.setExecutor(terraCommand);
            command.setTabCompleter(terraCommand);
        } else {
            getLogger().warning("Failed to register command: /s");
        }

        // Register // commands
        for (String alias : jinzo.terranite.commands.terraCommand.SUBCOMMANDS) {
            if ((command = getCommand("/" + alias)) != null) {
                AliasCommandExecutor aliasExec = new AliasCommandExecutor(alias);
                command.setExecutor(aliasExec);
                command.setTabCompleter(aliasExec);
            } else {
                getLogger().warning("Failed to register command: /" + alias);
            }
        }
        LegacyBlockHelper.loadLegacyBlocks();
